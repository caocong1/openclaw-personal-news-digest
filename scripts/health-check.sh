...
if [ $ALERT_RESULT -ne 0 ]; then
  echo "INFO: Could not check source failure history"
fi
...
b = json.load(open('$BASE_DIR/config/budget.json'))
...
# 9. Dedup-index consistency: orphaned entries (in dedup-index but not in any recent JSONL)
python3 -c "
import json, os, glob
from datetime import datetime, timedelta
...
for d in range(0, 8):
    date_str = (today - timedelta(days=d)).strftime('%Y-%m-%d')
    jsonl_path = os.path.join(base, 'data', 'news', f'{date_str}.jsonl')
...
" 2>/dev/null
if [ $? -ne 0 ]; then
  echo "INFO: Could not check dedup-index consistency"
fi

# 10. Source concentration alert: any single source > 50% of items fetched today
python3 -c "
import json, os
...
# Check per-source breakdown if available
...
" 2>/dev/null
if [ $? -ne 0 ]; then
  echo "INFO: Could not check source concentration"
fi
...
" 2>/dev/null
if [ $? -ne 0 ]; then
  echo "INFO: Could not check digest generation status"
fi
...
# Count unique URLs from last 7 days of JSONL
jsonl_urls = set()
today = datetime.now()
for d in range(0, 8):
    date_str = (today - timedelta(days=d)).strftime('%Y-%m-%d')
...
" 2>/dev/null
if [ $? -ne 0 ]; then
  echo "INFO: Could not check dedup-index rebuild status"
fi
...
" 2>/dev/null
if [ $? -ne 0 ]; then
  echo "INFO: Could not check empty events"
fi

# 14. Long-stable events: stable for > 14 days (candidates for manual review)
python3 -c "
import json, os
...
" 2>/dev/null
if [ $? -ne 0 ]; then
  echo "INFO: Could not check long-stable events"
fi
...
# Collect per-source stats from last 7 days of metrics
today = datetime.now()
source_stats = {}  # source_id -> {success: N, total: N}
for d in range(0, 8):
    date_str = (today - timedelta(days=d)).strftime('%Y-%m-%d')
...
" 2>/dev/null
if [ $? -ne 0 ]; then
  echo "INFO: Could not check source success rates"
fi
...
" 2>/dev/null
if [ $? -ne 0 ]; then
  echo "INFO: Could not check preference extremes"
fi
...
" 2>/dev/null
if [ $? -ne 0 ]; then
  echo "INFO: Could not check cache sizes"
fi
