...
5. **Handle errors**: On LLM failure, retry once. If still fails, mark `processing_status: "partial"` and call `bash {baseDir}/scripts/run-journal.sh append "$RUN_ID" error processing LLM_FAILURE "LLM call failed after 2 attempts for item $ITEM_ID" "Check API key, budget limits, or network connectivity" "$SOURCE_ID"` with the item_id and source_id in details. If classify fails but summarize succeeds, mark item for exploration slot. Any security block (e.g. malformed input that could indicate injection) calls `bash {baseDir}/scripts/run-journal.sh append "$RUN_ID" security processing SECURITY_BLOCK "Potential injection or security anomaly detected" "Audit the input source and content snippet"`.
