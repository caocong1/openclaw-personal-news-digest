...
4. **Fetch by type**: For each source, route by `source.type` using only agent-native fetch and extraction paths. `rss`: call `web_fetch(source.url, extractMode="text")` to fetch XML, then use LLM extraction over the XML text to map each RSS `<item>` or Atom `<entry>` into `title`, `url` from `<link>` or `href`, `published_at` from `<pubDate>` or `<published>`, and `content_snippet` from `<description>`, `<summary>`, or `<content>`. `github`: call `web_fetch` on the GitHub API JSON URL, then have the agent read the JSON response and map release or commit objects into `title`, `url`, `content_snippet`, and `published_at`. `search`: call `web_search` for configured keywords, collect title/url/snippet results, then apply the LLM search filter prompt before mapping kept results. `official`: call `web_fetch(source.url, extractMode="text")`, fall back to `browser(source.url)` when fetched text is insufficient or `prefer_browser` is true, then use the LLM extraction prompt for official announcements. `community`: call `browser(source.url)` as the primary renderer, then use the LLM extraction prompt for community posts. `ranking`: call `web_fetch(source.url, extractMode="text")`, fall back to `browser(source.url)` when ranking structure is missing or `prefer_browser` is true, then use the LLM extraction prompt for ranking lists. Use `{baseDir}/references/collection-instructions.md` only for field mappings, prompt names, error labels, and shared normalize/dedup/write details; this step is the executable routing contract. Track per-source counters (fetched, deduped, status, error) during collection for DailyMetrics `per_source` field. If a source fetch fails (timeout, network error, parse error), after writing the failure to per_source counters, also call `bash {baseDir}/scripts/run-journal.sh append "$RUN_ID" error collection SRC_TIMEOUT "Source $SOURCE_ID failed: $ERROR"` with the specific source_id and error in the message. If a fetch times out after 30s, record as `SRC_TIMEOUT`. If a fetch returns malformed data, record as `SRC_MALFORMED`. Then call `bash {baseDir}/scripts/run-journal.sh backlog "$RUN_ID" source_timeout "Source $SOURCE_ID failed: $ERROR" "Re-enable source manually or check network" "$SOURCE_ID"` for timeout failures, or `bash {baseDir}/scripts/run-journal.sh backlog "$RUN_ID" degraded_sources "Source $SOURCE_ID returned malformed data" "Check source URL and content format" "$SOURCE_ID"` for malformed data failures.
...
6b. **Freshness gate**: For each item surviving dedup, compare `published_at` (or `fetched_at` if `published_at` is null) against the current time. If the item is older than 24 hours, discard it immediately — do NOT write to JSONL. Increment `freshness_filtered` counter in per-source metrics. Emit run_log entry: step="freshness_gate", details={filtered_count, oldest_discarded_age_hours}. See `{baseDir}/references/collection-instructions.md` Section 2B for rules and edge cases.
7. **Write items**: Append new items to `{baseDir}/data/news/YYYY-MM-DD.jsonl` atomically via `exec` bash with a same-directory `mktemp` file: copy the existing target into the temp file if present, append the new JSONL lines, then `mv` the completed temp file over the target. The final file path and line format remain exactly `{baseDir}/data/news/YYYY-MM-DD.jsonl`. (Apply Pre-Write Quality Contract from `processing-instructions.md` Section 0D before writing.)
...
8. **Update dedup index**: Add new hashes to `{baseDir}/data/news/dedup-index.json` atomically via `exec` bash with a same-directory `mktemp` file, writing the complete updated JSON object and then renaming it over the target. Do not create or mutate `{baseDir}/data/alerts/alert-state-{today}.json` in Collection Phase. Emit run_log entry: step="collection_complete", details={sources_attempted, items_fetched, failed_sources}.
...
1. **Load prompts**: Read `{baseDir}/references/prompts/classify.md`, `{baseDir}/references/prompts/translate.md`, and `{baseDir}/references/prompts/summarize.md`.
...
3.6. **Translate batch**: For classified items with non-Chinese titles, group 5-10 items per LLM call using `{baseDir}/references/prompts/translate.md`. Store the returned `translated_title` on each `NewsItem` record alongside the original `title`; never overwrite `title`.
...
10. **Event merge**: For unique items, run event merge with LLM similarity assessment. For each plausible active or stable candidate event, ask whether the item and candidate describe the same underlying event, using the item's original `title`, `translated_title` when present, `content_summary`, `categories.primary`, and the candidate event title, summary, status, and timeline. Merge only when the LLM confirms the same underlying event; otherwise create a new event using the existing Event schema and lifecycle persistence contract. Emit run_log entry: step="dedup_complete".
